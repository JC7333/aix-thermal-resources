...
export const PdfDownloadButtons = ({
  ...
  variant = "default",
  className = "",
  showPreview = false, // Download-only par défaut (évite Chrome popup blocker)
  ...
  const [errorDetails, setErrorDetails] = useState<PdfErrorDetails | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const checkCache = () => {
      ...
      if (newStatus.page1 && !prevCacheRef.current.page1) {
        ...
        setTimeout(
          () => setJustCached((prev) => ({ ...prev, page1: false })),
          2000,
        );
      }
      if (newStatus.page4 && !prevCacheRef.current.page4) {
        ...
        setTimeout(
          () => setJustCached((prev) => ({ ...prev, page4: false })),
          2000,
        );
      }
  ...
  const handlePreview = async (type: "1page" | "4pages") => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Le PDF n’a pas pu être généré. Ouverture de la version imprimable.",
        variant: "destructive",
      });
  ...
  const handleDirectDownload = async (type: "1page" | "4pages") => {
    ...
    try {
      ...
      logEvent(
        type === "1page" ? "pdf_download_1page" : "pdf_download_4pages",
        `/pathologies/${slug}`,
        { slug, source: "direct" },
      );
      ...
      toast({
        ...
        description:
          "Ouverture de la version imprimable (Imprimer → Enregistrer en PDF).",
        variant: "destructive",
      });
  ...
  const handleDownloadFromPreview = () => {
    if (previewBlob) {
      ...
      logEvent(
        previewType === "1page" ? "pdf_download_1page" : "pdf_download_4pages",
        `/pathologies/${slug}`,
        {
          slug,
          source: "preview",
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Le PDF n’a pas pu être régénéré. Version imprimable disponible.",
        variant: "destructive",
      });
      ...
      <>
        <div className={`flex gap-2 ${className}`}>
          ...
          >
            ...
            {showReadyBadge && cacheStatus.page1 && !loading && (
              <ReadyBadge isNew={justCached.page1} />
            )}
          </Button>
          <Button
            variant="outline"
          ...
          >
            ...
            {showReadyBadge && cacheStatus.page4 && !loading && (
              <ReadyBadge isNew={justCached.page4} />
            )}
          </Button>
        </div>
        <PdfPreviewModal
        ...
        <PdfErrorDetailsModal
          ...
          onOpenPrintable={() =>
            openPrintableFallback({
              slug,
              variant: errorDetails?.variant ?? "1page",
              autoPrint: false,
            })
          }
          onPrint={() =>
  ...
  if (variant === "card") {
    return (
      <>
        <div
          className={`card-medical bg-primary/5 border-primary/20 ${className}`}
        >
